...
EASE = rx(r"\b(easy|easier|easily|ease|effortless\w*|simple|straight ?forward|seamless|smooth|breeze|painless|intuitive|hassle|user.?friendly|no problem)")
SPEED = rx(r"\b(quick|quickly|fast|prompt|promptly|speedy|speed|efficien\w*|soon|timely|expedit\w*|turn ?around|immediate\w*|within (a |an |\d+ |\w+ )?(day|days|hours?|week|weeks)|a (few|couple) (of )?days)\b")
DELAY_STRONG = rx(r"\b(still (in review|pending|waiting|not)|too long|long time|so long|took (a )?(long|forever|so long)|taking (a )?(long|forever)|forever|delay\w*|slow|lengthy|no update|long wait|never (paid|received))\b")
...
FRICTION = rx(r"(difficult|complicated|confusing|hard to|hassle|(have|had|has) to call|resubmit\w*|re-?submit\w*|sent back|missed|lost|errors?|mistakes?|incorrect|not being read|paperwork|additional (information|documents?)|new information|asked for (more|additional)|requested again|jump through|round and round)")
PAY = rx(r"\b(reimburs\w*|paid|payment|payout|pay(s)? out|deposit\w*|refund\w*|checks?|money)\b")
...
PREM = rx(r"((premium|rate|price|cost)s?\b[^.]{0,40}(increas\w*|went up|(go|goes|going|gone) up|hike\w*|rais\w*|skyrocket\w*|tripled|doubled|steep|too high|shot up|jump\w*)|(increas\w*|hike\w*|rais\w*)\b[^.]{0,20}(premium|rate|price|cost)s?|price hikes?)")
...
SERVICE_PRAISE = rx(r"\b(customer (service|care|support)|representatives?|agents?|associates?|advocates?|staff|phone|helpful|friendly|knowledgeable|courteous|polite|patient|wonderful people)\b")
...
def rule_themes(title, text, rating, n_words=None, service_praise=True):
    ...
    if CLAIM.search(t) and (DELAY_STRONG.search(t) or (low and DELAY.search(t))):
        hits.append((TURNAROUND, "claim_delay"))
    claim_ctx = bool(re.search(r"\b(claims?|submit\w*|filing|filed|file a|reimburs\w*|payouts?|pay(s)? out|refunds?|deposits?|checks)\b", t))
    if claim_ctx and low and FRICTION.search(t):
        hits.append((FILING, "claim_friction"))
    ...
    if service_praise and rating is not None and rating >= 4 and SERVICE_PRAISE.search(t):
        hits.append((GSS, "service_praise"))
...
CLAIM_JOURNEY = {FILING, TURNAROUND, REIMB}


def cap_hits(hits, max_themes=2):
    """Priority-ordered cap. A General/Mixed narrative flag rides on top of the cap.
    Normally at most `max_themes` specific themes are kept, but a THIRD is allowed when
    all kept themes and the extra one are claim-journey themes (Filing, Turnaround,
    Reimbursement), since those three describe one continuous experience."""
    kept, seen = [], set()
    if hits and hits[0][0] == GENERAL:
        kept.append(hits[0]); seen.add(GENERAL); hits = hits[1:]
    specific = []
    for th, rule in hits:
        if th in seen:
            continue
        n = len(specific)
        room = n < max_themes
        journey_extra = (n == max_themes and th in CLAIM_JOURNEY
                         and all(k[0] in CLAIM_JOURNEY for k in specific))
        if room or journey_extra:
            specific.append((th, rule)); seen.add(th)
    return kept + specific


def apply_rules(df, title="title", text="text", rating="rating", max_themes=2, service_praise=True):
    ...
    service_praise=False leaves generic service-praise reviews unresolved so centroid scoring
    can decide (useful for buckets where sign-up help and claims help are mixed)."""
    rows, cancel_ids = [], []
    for r in df.itertuples(index=False):
        hits, named, cancel = rule_themes(getattr(r, title), getattr(r, text), getattr(r, rating),
                                          service_praise=service_praise)
        kept = cap_hits(hits, max_themes)
