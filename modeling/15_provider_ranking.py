...
def build_flag_table(
    ...
    for _, row in df.iterrows():
        ...
        if row["theme"] in risk_themes and (is_index_hit or is_red_severity):
            signal_type = (
                "both"
                if (is_index_hit and is_red_severity)
                else ("severity" if is_red_severity else "frequency")
            )
            flags.append(
                {**row.to_dict(), "flag_type": "red", "signal_type": signal_type}
            )

        if row["theme"] in quality_themes and (is_index_hit or is_green_severity):
            signal_type = (
                "both"
                if (is_index_hit and is_green_severity)
                else ("severity" if is_green_severity else "frequency")
            )
            flags.append(
                {**row.to_dict(), "flag_type": "green", "signal_type": signal_type}
            )
...
def top_flags_by_provider(flags_df, n=2):
    ...
    return flags_df.groupby(["provider", "flag_type"]).head(n).reset_index(drop=True)
